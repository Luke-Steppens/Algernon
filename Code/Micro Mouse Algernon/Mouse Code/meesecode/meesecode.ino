...
// TCRT Thresholds
int front_Threshold = 1005;
int left_Threshold = 900;
int right_Threshold = 900;

int left_Drivespeed = 50;
int right_Drivespeed = 50;
...
void loop() {
  ...
  if (analogRead(TCRT_Front) < front_Threshold) {
    Serial.println("Front sensor triggered");
    Front = true;

  }
  if (analogRead(TCRT_Left) < left_Threshold) {
    Serial.println("Left sensor triggered");
    Left = true;

  }
  if (analogRead(TCRT_Right) < right_Threshold) {
...
void motorsForward() {
  left_Drivespeed = map(analogRead(TCRT_Left),500,1100,65,35);
  right_Drivespeed = map(analogRead(TCRT_Right),500,1100,65,35);
  analogWrite(motor_leftA, left_Drivespeed);
  analogWrite(motor_leftB, -left_Drivespeed);
  analogWrite(motor_rightA, right_Drivespeed);
  analogWrite(motor_rightB, -right_Drivespeed);
